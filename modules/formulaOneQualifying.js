...
export function getLastQualifyingTime(result = {}) {
  const recordedSeconds = Number(
    result.qualifying_unadjusted_seconds ?? result.qualifyingUnadjustedSeconds,
  );
  if (Number.isFinite(recordedSeconds) && recordedSeconds > 0) {
    return {
      session: "last",
      seconds: recordedSeconds,
      time: formatFormulaOneLapTime(recordedSeconds),
    };
  }
  for (const session of QUALIFYING_SESSIONS) {
    const seconds = parseFormulaOneLapTime(result[session]);
    if (seconds !== null)
      return { session, seconds, time: String(result[session]).trim() };
  }
  return null;
}
...
export function getAdjustedQualifyingTimes(first = {}, second = {}) {
  const firstRecorded = Number(
    first.qualifying_adjusted_seconds ?? first.qualifyingAdjustedSeconds,
  );
  const secondRecorded = Number(
    second.qualifying_adjusted_seconds ?? second.qualifyingAdjustedSeconds,
  );
  if (
    Number.isFinite(firstRecorded) &&
    firstRecorded > 0 &&
    Number.isFinite(secondRecorded) &&
    secondRecorded > 0
  ) {
    return {
      session: String(
        first.qualifying_adjusted_session ||
          second.qualifying_adjusted_session ||
          "adjusted",
      ),
      first: {
        seconds: firstRecorded,
        time: formatFormulaOneLapTime(firstRecorded),
      },
      second: {
        seconds: secondRecorded,
        time: formatFormulaOneLapTime(secondRecorded),
      },
    };
  }
  for (const session of QUALIFYING_SESSIONS) {
    ...
    if (firstSeconds !== null && secondSeconds !== null) {
      return {
        ...
        second: {
          ...
          time: String(second[session]).trim(),
        },
      };
    }
  }
...
export function buildFormulaOneQualifyingComparisons(
  results = [],
  drivers = [],
) {
  ...
  for (const result of qualifyingResults) {
    ...
    const driver = {
      ...storedDriver,
      constructor_id:
        result.constructor_id || storedDriver.constructor_id || "",
      constructor_name:
        result.constructor_name || storedDriver.constructor_name || "",
    };
  ...
  for (const group of roundGroups.values()) {
    ...
    const [first, second] = [...group].sort((a, b) =>
    ...
    comparisons.push({
      ...
      constructorId:
        first.driver.constructor_id || second.driver.constructor_id || "",
      constructorName:
        first.driver.constructor_name || second.driver.constructor_name || "",
  ...
  return comparisons.sort(
    (a, b) =>
      a.round - b.round || a.constructorName.localeCompare(b.constructorName),
  );
}
...
export function summarizeFormulaOneQualifyingComparisons(comparisons = []) {
  ...
  for (const comparison of comparisons) {
    const pairKey = [comparison.firstDriverId, comparison.secondDriverId]
      .sort()
      .join(":");
    ...
    if (comparison.adjusted)
      group.adjustedDifferences.push(comparison.adjusted.differenceSeconds);
    if (comparison.unadjusted)
      group.unadjustedDifferences.push(comparison.unadjusted.differenceSeconds);
  }
...
export function buildFormulaOneMainDatasets({
  year,
  rounds = [],
  drivers = [],
  sessions = [],
  ...
  const qualifyingComparisons = buildFormulaOneQualifyingComparisons(
    results,
    drivers,
  );
  const comparisonByRoundTeam = new Map(
    qualifyingComparisons.map((comparison) => [
  ...
  const teammateComparisons = driverPairs.map((pair) => {
    const comparison = comparisonByRoundTeam.get(
      `${pair.round}:${pair.constructorId || pair.constructorName}`,
    );
    const firstQualifying = resultByKey.get(
      `${pair.round}:qualifying:${pair.firstDriverId}`,
    );
    const secondQualifying = resultByKey.get(
      `${pair.round}:qualifying:${pair.secondDriverId}`,
    );
    const firstRace = resultByKey.get(
      `${pair.round}:race:${pair.firstDriverId}`,
    );
    const secondRace = resultByKey.get(
      `${pair.round}:race:${pair.secondDriverId}`,
    );
    return {
      year: Number(year),
      round: pair.round,
      team: pair.constructorName,
      firstDriver: pair.firstDriverName,
      secondDriver: pair.secondDriverName,
      firstDriverId: pair.firstDriverId,
      secondDriverId: pair.secondDriverId,
      unadjustedWinner: comparisonWinner(pair, comparison?.unadjusted),
      unadjustedGapSeconds: absoluteDifference(
        comparison?.unadjusted?.differenceSeconds,
      ),
      adjustedWinner: comparisonWinner(pair, comparison?.adjusted),
      adjustedGapSeconds: absoluteDifference(
        comparison?.adjusted?.differenceSeconds,
      ),
      adjustedSession: comparison?.adjusted?.session?.toUpperCase() || "",
      qualifyingPositionWinner: positionWinner(
        pair,
        firstQualifying,
        secondQualifying,
      ),
  ...
  for (const comparison of teammateComparisons) {
    comparisonByDriver.set(`${comparison.round}:${comparison.firstDriverId}`, {
      comparison,
      driverName: comparison.firstDriver,
    });
    comparisonByDriver.set(`${comparison.round}:${comparison.secondDriverId}`, {
      comparison,
      driverName: comparison.secondDriver,
    });
  }
  ...
  for (const result of results) {
    ...
    if (!participantIdsByRound.has(key))
      participantIdsByRound.set(key, new Set());
    participantIdsByRound.get(key).add(result.driver_id);
  }
  const mainData = rounds.flatMap((round) =>
    drivers
      .filter((driver) =>
        participantIdsByRound.get(Number(round.round))?.has(driver.driver_id),
      )
      .map((driver) => {
        const qualifying = resultByKey.get(
          `${round.round}:qualifying:${driver.driver_id}`,
        );
        const race = resultByKey.get(`${round.round}:race:${driver.driver_id}`);
        const qualifyingComparison = qualifyingComparisons.find(
          (comparison) =>
            Number(comparison.round) === Number(round.round) &&
            [comparison.firstDriverId, comparison.secondDriverId].includes(
              driver.driver_id,
            ),
        );
        const side =
          qualifyingComparison?.firstDriverId === driver.driver_id
            ? "first"
            : "second";
        const driverComparison = comparisonByDriver.get(
          `${round.round}:${driver.driver_id}`,
        );
        const unadjusted =
          qualifyingComparison?.unadjusted?.[side] ||
          getLastQualifyingTime(qualifying);
        ...
        return {
          ...
          team:
            qualifying?.constructor_name ||
            race?.constructor_name ||
            driver.constructor_name,
          racePoints: valueOrBlank(race?.points),
          racePodium: yesNo(race, numericPosition(race) <= 3),
          qualifyingUnadjustedTime: unadjusted?.time || "",
          qualifyingUnadjustedSeconds: valueOrBlank(unadjusted?.seconds),
          unadjustedQualifyingHeadToHead: headToHeadOutcome(
            driverComparison?.comparison.unadjustedWinner,
            driverComparison?.driverName,
          ),
          qualifyingAdjustedTime: adjusted?.time || "",
          qualifyingAdjustedSeconds: valueOrBlank(adjusted?.seconds),
          qualifyingAdjustedSession: adjusted?.session?.toUpperCase() || "",
          adjustedQualifyingHeadToHead: headToHeadOutcome(
            driverComparison?.comparison.adjustedWinner,
            driverComparison?.driverName,
          ),
          qualifyingPosition: sessionPosition(qualifying),
          qualifyingPositionHeadToHead: headToHeadOutcome(
            driverComparison?.comparison.qualifyingPositionWinner,
            driverComparison?.driverName,
          ),
          madeQ2: yesNo(
            qualifying,
            Boolean(qualifying?.q2) || numericPosition(qualifying) <= 15,
          ),
          madeQ3: yesNo(
            qualifying,
            Boolean(qualifying?.q3) || numericPosition(qualifying) <= 10,
          ),
          raceLapsCompleted: valueOrBlank(race?.laps),
          polePosition: yesNo(qualifying, numericPosition(qualifying) === 1),
          raceFinishingPosition: sessionPosition(race),
          raceFinishingPositionHeadToHead: headToHeadOutcome(
            driverComparison?.comparison.racePositionWinner,
            driverComparison?.driverName,
          ),
        };
      }),
  ...
  const roundSummary = rounds.map((round) => ({
    ...
    fastestPitTeam: round.fastest_pit_team || "",
    dnfCount: round.dnf_count || "",
    safetyCar: normalizeYesNo(round.safety_car),
  }));

  const sprintRounds = rounds.filter((round) => round.has_sprint);
  const sprintData = sprintRounds.flatMap((round) =>
    results
      .filter(
        (result) =>
          Number(result.round) === Number(round.round) &&
      ...
      .map((sprint) => {
        ...
        return {
          ...
          round: Number(round.round),
          roundName: round.name,
          driverId: sprint.driver_id,
          driver: driver.display_name || sprint.driver_id,
          team: sprint.constructor_name || driver.constructor_name,
          sprintPosition: valueOrBlank(sprint?.position),
          sprintPoints: valueOrBlank(sprint?.points),
          adjustedSprintPoints: sprint
            ? getRacePointsForPosition(numericPosition(sprint))
            : "",
        };
      }),
  );
  ...
  const sprintSummary = sprintRounds.map((round) => {
    const sprintWinner = results.find(
      (result) =>
        Number(result.round) === Number(round.round) &&
        result.session_type === "sprint" &&
    ...
    return {
      ...
      roundName: round.name,
      sprintWinner:
        driverById.get(sprintWinner?.driver_id)?.display_name ||
        sprintWinner?.driver_id ||
        "",
      raceWinner:
        driverById.get(raceWinner?.driver_id)?.display_name ||
        raceWinner?.driver_id ||
        "",
      sameRaceAndSprintWinner:
        sprintWinner && raceWinner
          ? sprintWinner.driver_id === raceWinner.driver_id
            ? "Yes"
            : "No"
          : "",
    };
...
export function parseFormulaOneCalculatorData({
  driversCsv,
  optionsCsv,
  sprintsCsv,
  summaryCsv,
  ...
  const pointTables = findCalculatorPointTables(optionsRows);
  const raceOptions = pointTables.find((table) =>
    table.some((option) => option.position === "<10"),
  );
  const sprintOptions = pointTables.find((table) =>
    table.some((option) => option.position === "<8"),
  );
  const driversToWatch = findDriversToWatch(optionsRows, pointTables);

  if (
    !raceOptions?.length ||
    !sprintOptions?.length ||
    !driversToWatch.length
  ) {
    throw new Error(
      "Formula 1 calculator options did not include RacePoints, SprintPoints, and DriversToWatch data.",
    );
  }

  const raceData = parseCalculatorRoundTable(driversCsv);
  const sprintData = parseCalculatorRoundTable(sprintsCsv);
  const currentTotals = parseCalculatorSummary(summaryCsv);
...
function findCalculatorPointTables(rows) {
  const tables = [];
  ...
  rows.forEach((row, rowIndex) => {
    row.forEach((value, columnIndex) => {
      if (
        ...
        normalize(row[columnIndex + 1]) !== "points"
      ) {
        return;
      }
      ...
      for (const optionRow of rows.slice(rowIndex + 1)) {
        ...
        if (!position && !pointsText) break;
        const points = Number(pointsText.replace(/,/g, ""));
        if (!position || !Number.isFinite(points)) break;
...
function findDriversToWatch(rows, pointTables) {
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
    ...
    if (!passedPointTables) continue;

    const driverColumn = row.findIndex(
      (value) => normalize(value) === "driver",
    );
    if (driverColumn < 0) continue;
...
function parseCalculatorRoundTable(csvText) {
  const rows = parseCsvMatrix(csvText);
  const headerIndex = rows.findIndex(
    (row) =>
      normalize(row[0]) === "driver" &&
      row.some((value) => /^round\s+\d+/i.test(String(value ?? "").trim())),
  );
  if (headerIndex < 0) return { rounds: [] };

  const headers = rows[headerIndex];
  const prettyHeaders = rows[headerIndex - 1] ?? [];
  const totalColumn = headers.findIndex(
    (value) => normalize(value) === "total",
  );
  ...
  for (const row of rows.slice(headerIndex + 1)) {
    ...
    if (!driver || normalize(driver) === "count") break;
    roundColumns.forEach((round) => {
      const value = String(row[round.columnIndex] ?? "").trim();
      round.pointsByDriver.set(normalize(driver), value);
    });
  }

  roundColumns.forEach((round) => {
    round.complete = [...round.pointsByDriver.values()].some(
      (value) => value !== "",
    );
  });
...
function parseCalculatorSummary(csvText) {
  const rows = parseCsvMatrix(csvText);
  const headerIndex = rows.findIndex(
    (row) =>
      normalize(row[0]) === "driver" &&
      row.some((value) => normalize(value) === "total"),
  );
  const currentTotals = new Map();
  if (headerIndex < 0) return currentTotals;

  const totalColumn = rows[headerIndex].findIndex(
    (value) => normalize(value) === "total",
  );
  for (const row of rows.slice(headerIndex + 1)) {
    const driver = String(row[0] ?? "").trim();
    ...
    currentTotals.set(
      normalize(driver),
      calculatorPointNumber(row[totalColumn]),
    );
  }
...
function buildRoundDriverPairs(results, driverById) {
  ...
  for (const result of results) {
    ...
    if (!groups.has(key))
      groups.set(key, {
        ...
        constructorId,
        constructorName:
          result.constructor_name || driver.constructor_name || "",
        driverIds: new Set(),
      });
  ...
  return [...groups.values()]
    .flatMap((group) => {
      ...
      const [firstDriverId, secondDriverId] = [...group.driverIds].sort(
        (firstId, secondId) =>
          (driverById.get(firstId)?.display_name || firstId).localeCompare(
            driverById.get(secondId)?.display_name || secondId,
          ),
      );
      return [
        {
          ...
          firstDriverId,
          firstDriverName:
            driverById.get(firstDriverId)?.display_name || firstDriverId,
          secondDriverId,
          secondDriverName:
            driverById.get(secondDriverId)?.display_name || secondDriverId,
        },
      ];
    })
    .sort(
...
function comparisonWinner(comparison, values) {
  ...
  return values.differenceSeconds < 0
    ? comparison.firstDriverName
    : comparison.secondDriverName;
}
...
function positionGap(first, second) {
  const firstPosition = numericPosition(first);
  const secondPosition = numericPosition(second);
  return Number.isFinite(firstPosition) && Number.isFinite(secondPosition)
    ? Math.abs(firstPosition - secondPosition)
    : "";
}
