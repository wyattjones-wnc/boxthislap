...
export function createTrophyLogController({ endpoint, getAccessToken }) {
  ...
  const swapDialog = document.querySelector("#featured-platinum-swap-dialog");
  const swapIntro = document.querySelector("#featured-platinum-swap-intro");
  const swapOptions = document.querySelector("#featured-platinum-swap-options");
  const swapStatus = document.querySelector("#featured-platinum-swap-status");
  const swapConfirm = document.querySelector("#featured-platinum-swap-confirm");
  const state = {
    view: "unsorted",
    sort: "newest",
    evergreenSort: false,
  ...
  filterToggle?.addEventListener("click", () => {
    ...
    filterToggle.setAttribute(
      "aria-label",
      `${expanded ? "Show" : "Hide"} trophy filters and sorting`,
    );
  ...
  sortSelect?.addEventListener("change", () => {
    ...
    state.view = state.sort.startsWith("platinum-duration-")
      ? "platinums"
      : "all";
    syncFilters();
    state.page = 1;
  ...
  grid?.addEventListener("click", (event) => {
    const seenThroughButton = event.target.closest(
      "[data-trophy-seen-through]",
    );
    if (seenThroughButton) {
      if (!seenThroughButton.classList.contains("is-confirming")) {
        grid
          .querySelectorAll("[data-trophy-seen-through].is-confirming")
          .forEach((button) => {
        ...
        seenThroughButton.querySelector("span").textContent =
          "Confirm through here";
        syncStatus.textContent =
          "Click again to mark this trophy and every earlier unsorted trophy across all pages Not Favorite.";
        return;
      }
    ...
    const featuredButton = event.target.closest("[data-trophy-featured]");
    if (featuredButton) void toggleFeatured(featuredButton);
  });
  swapOptions?.addEventListener("click", (event) => {
    const button = event.target.closest("[data-featured-replacement]");
    if (!button) return;
    swapReplacement = {
      gameId: button.dataset.gameId,
      trophyId: Number(button.dataset.trophyId),
    };
    swapOptions
      .querySelectorAll("[data-featured-replacement]")
      .forEach((option) => {
  ...
  swapConfirm?.addEventListener("click", () => void confirmSwap());
  swapDialog
    ?.querySelectorAll("[data-featured-swap-close]")
    .forEach((button) => button.addEventListener("click", closeSwapDialog));
  swapDialog?.addEventListener("click", (event) => {
    if (event.target === swapDialog) closeSwapDialog();
  });
  ...
  function renderPage() {
    const requestedView = sessionStorage.getItem("boxThisLapTrophyLogView");
    if (requestedView === "platinums") {
      state.view = requestedView;
      state.sort = "newest";
      state.evergreenSort = false;
      state.page = 1;
      sessionStorage.removeItem("boxThisLapTrophyLogView");
    }
    syncFilters();
    return Promise.all([load(), loadAuthStatus(), loadFeatured()]);
  }
  ...
  async function load(force = false) {
    ...
    try {
      const value = await request(
        `/api/psn/trophy-log?view=${encodeURIComponent(state.view)}&sort=${encodeURIComponent(state.sort)}&evergreen=${state.evergreenSort}&page=${state.page}&limit=48`,
      );
  ...
  function renderPagination(value) {
    ...
    pagination.innerHTML =
      !hasPrevious && !hasMore
        ? ""
        : `<button type="button" data-trophy-log-page="${state.page - 1}" ${hasPrevious ? "" : "disabled"}>Previous</button><span>Page ${state.page}</span><button type="button" data-trophy-log-page="${state.page + 1}" ${hasMore ? "" : "disabled"}>Next</button>`;
  }
  ...
  async function savePreference(button) {
    ...
    try {
      ...
      await request(
        `/api/psn/trophies/${encodeURIComponent(card.dataset.gameId)}/${encodeURIComponent(card.dataset.trophyId)}/preference`,
        {
          method: "PUT",
      ...
      const value = await request("/api/psn/trophies/seen-through", {
        ...
        body: JSON.stringify({
          anchor: {
            ...
            trophyId: Number(card.dataset.trophyId),
          },
          view: state.view,
          sort: state.sort,
  ...
  function updatePreferenceInPlace(card, nextState) {
    ...
    const leavesCurrentView =
      (state.view === "unsorted" && nextState !== null) ||
    ...
    if (actions)
      actions.innerHTML = renderActions(item || { state: current }, state.view);
  }

  async function loadFeatured(force = false) {
    if (featuredLoaded && !force) return featuredItems;
    const value = await request("/api/psn/featured-platinums");
  ...
  async function toggleFeatured(button) {
    ...
    const item = state.items.find(
      (entry) =>
        entry.gameId === card.dataset.gameId &&
        Number(entry.id) === Number(card.dataset.trophyId),
    );
    if (!item || item.type !== "platinum") return;
    button.disabled = true;
    try {
      await loadFeatured();
      if (item.featured) {
        await saveFeatured(item, false);
        return;
      }
      ...
      await saveFeatured(item, true);
    } catch (error) {
      if (/Trophy Case is full/i.test(error.message)) {
  ...
  async function saveFeatured(item, featured, replace = null) {
    await request(
      `/api/psn/featured-platinums/${encodeURIComponent(item.gameId)}/${encodeURIComponent(item.id)}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ featured, ...(replace ? { replace } : {}) }),
      },
    );
    pageCache.clear();
    if (replace) {
  ...
  function openSwapDialog(item) {
    if (!swapDialog || !swapOptions || !swapConfirm) return;
    swapIncoming = item;
    swapReplacement = null;
    swapConfirm.disabled = true;
    swapStatus.textContent = "";
    swapIntro.textContent = `Choose which featured platinum to replace with ${item.name || item.gameName}.`;
    swapOptions.innerHTML = featuredItems
      .map(
        (entry) =>
          `<button type="button" data-featured-replacement data-game-id="${escapeHtml(entry.gameId)}" data-trophy-id="${escapeHtml(entry.id)}" aria-pressed="false"><img src="${escapeHtml(entry.iconUrl || "")}" alt="" loading="lazy"><span><strong>${escapeHtml(entry.gameName || entry.name)}</strong><small>Platinum #${escapeHtml(entry.platinumNumber || "")}</small></span></button>`,
      )
      .join("");
  ...
  async function confirmSwap() {
    if (!swapIncoming || !swapReplacement || !swapConfirm) return;
    swapConfirm.disabled = true;
    swapStatus.textContent = "Updating Trophy Case...";
    try {
      await saveFeatured(swapIncoming, true, swapReplacement);
      closeSwapDialog();
    } catch (error) {
  ...
  async function refreshTrophies() {
    ...
    syncStatus.textContent =
      "Checking PSN for changed and recently played games...";
    try {
  ...
  async function loadAuthStatus() {
    ...
    try {
      ...
      authStatus.textContent = value.updatedAt
        ...
        : value.configured
          ? "PSN access is configured. You can replace it below when Sony expires it."
          : "PSN access is not configured.";
    } catch (error) {
  ...
  async function savePsnAuth() {
    ...
    if (!npsso) {
      authStatus.textContent = "Paste the NPSSO value first.";
      input?.focus();
      return;
    }
...
function loading(label) {
  return `<p class="table-message"><span class="loading-spinner"></span>${escapeHtml(label)}</p>`;
}
...
function renderActions(item, view) {
  const current = item.state || "unsorted";
  const favorite = current === "favorite";
  const featured =
    item.type === "platinum"
      ? `<button type="button" data-trophy-featured="${item.featured ? "true" : "false"}" class="${item.featured ? "is-active" : ""}" aria-pressed="${String(Boolean(item.featured))}">${item.featured ? "Unfeature" : "Feature"}</button>`
      : "";
  return `<button type="button" data-trophy-preference="${favorite ? "seen" : "favorite"}">${favorite ? "★ Unfavorite" : "☆ Favorite"}</button>${featured}${current === "unsorted" && view === "unsorted" ? `<button class="action-button trophy-seen-through-button" type="button" data-trophy-seen-through aria-expanded="false" aria-label="Seen through here"><svg aria-hidden="true" viewBox="0 0 24 24" focusable="false"><path d="M12 19V5m-6 6 6-6 6 6"></path></svg><span>Seen through here</span></button>` : ""}`;
}
