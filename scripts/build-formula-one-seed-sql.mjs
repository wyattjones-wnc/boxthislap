...
for (const result of data.results || []) {
  statements.push(`INSERT INTO f1_session_results (year, round, session_type, driver_id, position, classified_position, constructor_id, constructor_name, grid, points, laps, status, q1, q2, q3, time_text, fastest_lap_rank, qualifying_unadjusted_seconds, qualifying_adjusted_seconds, qualifying_adjusted_session, raw_json, updated_at)
VALUES (${year}, ${integer(result.round)}, ${text(result.sessionType)}, ${text(result.driverId)}, ${nullableNumber(result.position)}, ${text(result.classifiedPosition)}, ${text(result.constructorId)}, ${text(result.constructorName)}, NULL, ${number(result.points)}, ${nullableNumber(result.laps)}, ${text(result.status)}, '', '', '', '', NULL, ${nullableNumber(result.qualifyingUnadjustedSeconds)}, ${nullableNumber(result.qualifyingAdjustedSeconds)}, '', '{}', CURRENT_TIMESTAMP)
ON CONFLICT(year, round, session_type, driver_id) DO UPDATE SET position = excluded.position, classified_position = excluded.classified_position, constructor_id = excluded.constructor_id, constructor_name = excluded.constructor_name, points = excluded.points, laps = excluded.laps, status = excluded.status, qualifying_unadjusted_seconds = excluded.qualifying_unadjusted_seconds, qualifying_adjusted_seconds = excluded.qualifying_adjusted_seconds, updated_at = CURRENT_TIMESTAMP;`);
}
